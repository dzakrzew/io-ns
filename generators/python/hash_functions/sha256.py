class SHA256:
    ...
    def digest(input_data):
        ...
        def SSIG0(num: int):
            num = (ROTR(num, 7) ^ ROTR(num, 18) ^ (num >> 3))
            return num

        def SSIG1(num: int):
            num = (ROTR(num, 17) ^ ROTR(num, 19) ^ (num >> 10))
            return num

        def BSIG0(num: int):
            num = (ROTR(num, 2) ^ ROTR(num, 13) ^ ROTR(num, 22))
            return num

        def BSIG1(num: int):
            num = (ROTR(num, 6) ^ ROTR(num, 11) ^ ROTR(num, 25))
            return num
        ...
        blocks = [] # 512-bits = 64 bytes chunks of message
        ...
        for message_block in blocks:
            ...
            for t in range(64):
                t1 = ((h + BSIG1(e) + CH(e, f, g) + K[t] + int.from_bytes(message_schedule[t], 'big')) % 2**32)
        ...
        result = h0.to_bytes(4, 'big') + h1.to_bytes(4, 'big') + h2.to_bytes(4, 'big') + h3.to_bytes(4, 'big') + h4.to_bytes(4, 'big') + h5.to_bytes(4, 'big') + h6.to_bytes(4, 'big') + h7.to_bytes(4, 'big')
