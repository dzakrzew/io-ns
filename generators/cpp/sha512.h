...
class SHA512 {
    private:
        ...
        std::string getHex(uint64_t in);
        std::vector<uint64_t> resizeBlock(std::vector<uint64_t> in);
    public:
        std::string digest(std::string input);
};

#define SHA512_ROTR(word,bits) (((word) >> (bits)) | ((word) << (64-(bits))))
...
#define SHA512_MOD(x) ((x) & 0xFFFFFFFFFFFFFFFF)